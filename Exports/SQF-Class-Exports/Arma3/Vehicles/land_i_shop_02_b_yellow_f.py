d = {
    "_generalmacro": "Land_i_Shop_02_b_yellow_F",
    "access": 0,
    "accuracy": 1000,
    "accuracydarknightlightsoff": 0.001,
    "accuracynightlightsoff": 0.006,
    "accuracynightlightson": 0.1,
    "actionbegin1": "OpenDoor_1",
    "actionbegin2": "OpenDoor_2",
    "actionbegin3": "OpenDoor_3",
    "actionbegin4": "OpenDoor_4",
    "actionend1": "OpenDoor_1",
    "actionend2": "OpenDoor_2",
    "actionend3": "OpenDoor_3",
    "actionend4": "OpenDoor_4",
    "aggregatereflectors": [],
    "aircapacity": 10,
    "allowtablock": 1,
    "alwaystarget": 0,
    "animated": 0,
    "animationsources": {
        "door_1_locked_source": {
            "animperiod": 0.8,
            "initphase": 0,
            "source": "user"
        },
        "door_1_nosound_source": {
            "animperiod": 1,
            "initphase": 0,
            "source": "user"
        },
        "door_1_sound_source": {
            "animperiod": 1,
            "initphase": 0,
            "sound": "GenericDoorsSound",
            "soundposition": "Door_1_trigger",
            "source": "user"
        },
        "door_2_locked_source": {
            "animperiod": 0.8,
            "initphase": 0,
            "source": "user"
        },
        "door_2_nosound_source": {
            "animperiod": 1,
            "initphase": 0,
            "source": "user"
        },
        "door_2_sound_source": {
            "animperiod": 1,
            "initphase": 0,
            "sound": "GenericDoorsSound",
            "soundposition": "Door_2_trigger",
            "source": "user"
        },
        "door_3_locked_source": {
            "animperiod": 0.8,
            "initphase": 0,
            "source": "user"
        },
        "door_3_nosound_source": {
            "animperiod": 1,
            "initphase": 0,
            "source": "user"
        },
        "door_3_sound_source": {
            "animperiod": 1,
            "initphase": 0,
            "sound": "GenericDoorsSound",
            "soundposition": "Door_3_trigger",
            "source": "user"
        },
        "door_4_locked_source": {
            "animperiod": 0.8,
            "initphase": 0,
            "source": "user"
        },
        "door_4_nosound_source": {
            "animperiod": 1,
            "initphase": 0,
            "source": "user"
        },
        "door_4_sound_source": {
            "animperiod": 1,
            "initphase": 0,
            "sound": "GenericDoorsSound",
            "soundposition": "Door_4_trigger",
            "source": "user"
        },
        "glass_10_source": {
            "hitpoint": "Glass_10_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_11_source": {
            "hitpoint": "Glass_11_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_12_source": {
            "hitpoint": "Glass_12_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_13_source": {
            "hitpoint": "Glass_13_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_14_source": {
            "hitpoint": "Glass_14_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_15_source": {
            "hitpoint": "Glass_15_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_16_source": {
            "hitpoint": "Glass_16_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_17_source": {
            "hitpoint": "Glass_17_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_18_source": {
            "hitpoint": "Glass_18_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_19_source": {
            "hitpoint": "Glass_19_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_1_source": {
            "hitpoint": "Glass_1_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_20_source": {
            "hitpoint": "Glass_20_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_21_source": {
            "hitpoint": "Glass_21_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_2_source": {
            "hitpoint": "Glass_2_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_3_source": {
            "hitpoint": "Glass_3_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_4_source": {
            "hitpoint": "Glass_4_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_5_source": {
            "hitpoint": "Glass_5_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_6_source": {
            "hitpoint": "Glass_6_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_7_source": {
            "hitpoint": "Glass_7_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_8_source": {
            "hitpoint": "Glass_8_hitpoint",
            "raw": 1,
            "source": "Hit"
        },
        "glass_9_source": {
            "hitpoint": "Glass_9_hitpoint",
            "raw": 1,
            "source": "Hit"
        }
    },
    "antirollbarforcecoef": 0,
    "antirollbarforcelimit": 5,
    "antirollbarspeedmax": 60,
    "antirollbarspeedmin": 20,
    "armor": 2000,
    "armorlights": 0.4,
    "armorstructural": 1,
    "artilleryscanner": 0,
    "artillerytarget": 0,
    "attendant": 0,
    "attenuationeffecttype": "",
    "audible": 0,
    "author": "Bohemia Interactive",
    "autocenter": 1,
    "brakedistance": 5,
    "cabinclosesound": [
        "",
        1,
        1
    ],
    "cabinclosesoundinternal": [
        "",
        1,
        1
    ],
    "cabinopensound": [
        "",
        1,
        1
    ],
    "cabinopensoundinternal": [
        "",
        1,
        1
    ],
    "camerasmoothspeed": 5,
    "camouflage": 2,
    "camshakedamage": {
        "attenuation": 0.5,
        "distance": -1,
        "duration": 3,
        "frequency": 60,
        "minspeed": 1,
        "power": 0.5
    },
    "camshakegforce": {
        "distance": 0,
        "duration": 3,
        "frequency": 20,
        "minspeed": 1,
        "power": 1
    },
    "canhidedriver": -1,
    "canusescanners": 1,
    "cargoaction": [],
    "cargocaneject": 1,
    "cargocompartments": [
        "Compartment1"
    ],
    "cargogetinaction": [],
    "cargogetoutaction": [],
    "cargoiscodriver": [
        0
    ],
    "cargospec": {
        "cargo1": {
            "showheadphones": 0
        }
    },
    "castcargoshadow": 0,
    "castdrivershadow": 0,
    "coefinside": 2,
    "coefinsideheur": 2,
    "coefspeedinside": 2,
    "commandercansee": 31,
    "cost": 40000,
    "countsforscoreboard": 0,
    "crew": "Civilian",
    "crewcrashprotection": 1,
    "crewvulnerable": 1,
    "damage": {
        "mat": [
            "A3/Structures_F/Data/Windows/window_set.rvmat",
            "A3/Structures_F/Data/Windows/destruct_half_window_set.rvmat",
            "A3/Structures_F/Data/Windows/destruct_full_window_set.rvmat",
            "A3/Structures_F_Argo/Commercial/Shop_02/Data/i_Shop_02_b_add_yellow_multi.rvmat",
            "A3/Structures_F_Argo/Commercial/Shop_02/Data/destruct_i_Shop_02_b_add_yellow_multi.rvmat",
            "A3/Structures_F_Argo/Commercial/Shop_02/Data/destruct_i_Shop_02_b_add_yellow_multi.rvmat",
            "A3/Structures_F_Argo/Commercial/Shop_02/Data/i_Shop_02_b_yellow_multi.rvmat",
            "A3/Structures_F_Argo/Commercial/Shop_02/Data/destruct_i_Shop_02_b_yellow_multi.rvmat",
            "A3/Structures_F_Argo/Commercial/Shop_02/Data/destruct_i_Shop_02_b_yellow_multi.rvmat"
        ],
        "tex": [
            "A3/Structures_F/Data/Windows/window_set_CA.paa",
            "A3/Structures_F/Data/Windows/destruct_half_window_set_CA.paa"
        ]
    },
    "damagefull": [],
    "damagehalf": [],
    "damageresistance": 0.004,
    "damagetexdelay": 0,
    "destrtype": "DestructBuilding",
    "destructioneffects": {
        "damagearound1": {
            "intensity": 1,
            "interval": 1,
            "lifetime": 1,
            "position": "",
            "simulation": "damageAround",
            "type": "DamageAroundHouse"
        },
        "destroyphase1": {
            "intensity": 1,
            "interval": 1,
            "lifetime": 2.5,
            "position": "",
            "simulation": "destroy",
            "type": "DelayedDestruction"
        },
        "ruin1": {
            "intensity": 1,
            "interval": 1,
            "lifetime": 1,
            "position": "",
            "simulation": "ruin",
            "type": "A3/Structures_F_Argo/Commercial/Shop_02/Shop_02_b_yellow_ruins_F.p3d"
        },
        "smoke1": {
            "intensity": 0.15,
            "interval": 1,
            "lifetime": 0.05,
            "position": "destructionEffect1",
            "qualitylevel": 2,
            "simulation": "particles",
            "type": "HouseDestructionSmoke3"
        },
        "smoke1low": {
            "intensity": 0.15,
            "interval": 1,
            "lifetime": 0.05,
            "position": "destructionEffect1",
            "qualitylevel": 0,
            "simulation": "particles",
            "type": "HouseDestructionSmoke3Low"
        },
        "smoke1med": {
            "intensity": 0.15,
            "interval": 1,
            "lifetime": 0.05,
            "position": "destructionEffect1",
            "qualitylevel": 1,
            "simulation": "particles",
            "type": "HouseDestructionSmoke3Med"
        },
        "smoke2": {
            "intensity": 0.15,
            "interval": 1,
            "lifetime": 0.035,
            "position": "destructionEffect1",
            "qualitylevel": 2,
            "simulation": "particles",
            "type": "HouseDestructionSmoke4"
        },
        "smoke2low": {
            "intensity": 0.15,
            "interval": 1,
            "lifetime": 0.035,
            "position": "destructionEffect1",
            "qualitylevel": 0,
            "simulation": "particles",
            "type": "HouseDestructionSmoke4Low"
        },
        "smoke2med": {
            "intensity": 0.15,
            "interval": 1,
            "lifetime": 0.035,
            "position": "destructionEffect1",
            "qualitylevel": 1,
            "simulation": "particles",
            "type": "HouseDestructionSmoke4Med"
        },
        "smoke3": {
            "intensity": 0.15,
            "interval": 1,
            "lifetime": 0.035,
            "position": "destructionEffect1",
            "qualitylevel": 2,
            "simulation": "particles",
            "type": "HouseDestrSmokeLong"
        },
        "smoke3low": {
            "intensity": 0.15,
            "interval": 1,
            "lifetime": 0.035,
            "position": "destructionEffect1",
            "qualitylevel": 0,
            "simulation": "particles",
            "type": "HouseDestrSmokeLongLow"
        },
        "smoke3med": {
            "intensity": 0.15,
            "interval": 1,
            "lifetime": 0.035,
            "position": "destructionEffect1",
            "qualitylevel": 1,
            "simulation": "particles",
            "type": "HouseDestrSmokeLongMed"
        },
        "sound": {
            "intensity": 1,
            "interval": 1,
            "lifetime": 0.125,
            "position": "destructionEffect1",
            "simulation": "sound",
            "type": "DestrHouse"
        }
    },
    "detectskill": 20,
    "displayname": "Shop (Yellow & White)",
    "dlc": "Argo",
    "driveraction": "",
    "drivercaneject": 1,
    "drivercansee": "2+8+16",
    "drivercompartments": "Compartment1",
    "driverforceoptics": 0,
    "driverinaction": "",
    "driveropticscolor": [
        1,
        1,
        1,
        1
    ],
    "driveropticseffect": [],
    "driveropticsmodel": "",
    "dustbackleftpos": "dustBackLeft",
    "dustbackrightpos": "dustBackRight",
    "dustfrontleftpos": "dustFrontLeft",
    "dustfrontrightpos": "dustFrontRight",
    "editorcategory": "EdCat_Structures_Malden",
    "editorpreview": "A3/EditorPreviews_F_Argo/Data/CfgVehicles/Land_i_Shop_02_b_yellow_F.jpg",
    "editorsubcategory": "EdSubcat_Residential_City",
    "ejectdeadcargo": 0,
    "ejectdeaddriver": 0,
    "emptysound": [
        "",
        0,
        1
    ],
    "enablegps": 0,
    "enablemanualfire": 1,
    "enableradio": 0,
    "enablewatch": 0,
    "engineer": 0,
    "epeimpulsedamagecoef": 30,
    "eventhandlers": {},
    "explosionshielding": 5,
    "extcameraparams": [
        1
    ],
    "extcameraposition": [
        0,
        2,
        -20
    ],
    "faction": "Default",
    "features": "",
    "featuretype": 0,
    "fireresistance": 10,
    "forcehidedriver": 0,
    "formationtime": 5,
    "formationx": 10,
    "formationz": 20,
    "fuelcapacity": 0,
    "fuelconsumptionrate": 0.01,
    "fxexplo": {
        "access": 1
    },
    "getinaction": "",
    "getinradius": 2.5,
    "getoutaction": "",
    "gforceshakeattenuation": 0.5,
    "ghostpreview": "",
    "groupcameraposition": [
        0,
        5,
        -30
    ],
    "gunclouds": {
        "access": 0,
        "cloudletaccy": 0.4,
        "cloudletalpha": 1,
        "cloudletanimperiod": 1,
        "cloudletcolor": [
            1,
            1,
            1,
            0
        ],
        "cloudletduration": 0.3,
        "cloudletfadein": 0.01,
        "cloudletfadeout": 1,
        "cloudletgrowup": 1,
        "cloudletmaxyspeed": 0.8,
        "cloudletminyspeed": 0.2,
        "cloudletshape": "cloudletClouds",
        "cloudletsize": 1,
        "deltat": 0,
        "initt": 0,
        "interval": 0.05,
        "size": 3,
        "sourcesize": 0.5,
        "table": {
            "t0": {
                "color": [
                    1,
                    1,
                    1,
                    0
                ],
                "maxt": 0
            }
        },
        "timetolive": 0
    },
    "gunfire": {
        "access": 0,
        "cloudletaccy": 0,
        "cloudletalpha": 1,
        "cloudletanimperiod": 1,
        "cloudletcolor": [
            1,
            1,
            1,
            0
        ],
        "cloudletduration": 0.2,
        "cloudletfadein": 0.01,
        "cloudletfadeout": 0.5,
        "cloudletgrowup": 0.2,
        "cloudletmaxyspeed": 100,
        "cloudletminyspeed": -100,
        "cloudletshape": "cloudletFire",
        "cloudletsize": 1,
        "deltat": -3000,
        "initt": 4500,
        "interval": 0.01,
        "size": 3,
        "sourcesize": 0.5,
        "table": {
            "t0": {
                "color": [
                    0.82,
                    0.95,
                    0.93,
                    0
                ],
                "maxt": 0
            },
            "t1": {
                "color": [
                    0.75,
                    0.77,
                    0.9,
                    0
                ],
                "maxt": 200
            },
            "t10": {
                "color": [
                    0.62,
                    0.29,
                    0.03,
                    0
                ],
                "maxt": 2600
            },
            "t11": {
                "color": [
                    0.59,
                    0.35,
                    0.05,
                    0
                ],
                "maxt": 2650
            },
            "t12": {
                "color": [
                    0.75,
                    0.37,
                    0.03,
                    0
                ],
                "maxt": 2700
            },
            "t13": {
                "color": [
                    0.88,
                    0.34,
                    0.03,
                    0
                ],
                "maxt": 2750
            },
            "t14": {
                "color": [
                    0.91,
                    0.5,
                    0.17,
                    0
                ],
                "maxt": 2800
            },
            "t15": {
                "color": [
                    1,
                    0.6,
                    0.2,
                    0
                ],
                "maxt": 2850
            },
            "t16": {
                "color": [
                    1,
                    0.71,
                    0.3,
                    0
                ],
                "maxt": 2900
            },
            "t17": {
                "color": [
                    0.98,
                    0.83,
                    0.41,
                    0
                ],
                "maxt": 2950
            },
            "t18": {
                "color": [
                    0.98,
                    0.91,
                    0.54,
                    0
                ],
                "maxt": 3000
            },
            "t19": {
                "color": [
                    0.98,
                    0.99,
                    0.6,
                    0
                ],
                "maxt": 3100
            },
            "t2": {
                "color": [
                    0.56,
                    0.62,
                    0.67,
                    0
                ],
                "maxt": 400
            },
            "t20": {
                "color": [
                    0.96,
                    0.99,
                    0.72,
                    0
                ],
                "maxt": 3300
            },
            "t21": {
                "color": [
                    1,
                    0.98,
                    0.91,
                    0
                ],
                "maxt": 3600
            },
            "t22": {
                "color": [
                    1,
                    1,
                    1,
                    0
                ],
                "maxt": 4200
            },
            "t3": {
                "color": [
                    0.39,
                    0.46,
                    0.47,
                    0
                ],
                "maxt": 600
            },
            "t4": {
                "color": [
                    0.24,
                    0.31,
                    0.31,
                    0
                ],
                "maxt": 800
            },
            "t5": {
                "color": [
                    0.23,
                    0.31,
                    0.29,
                    0
                ],
                "maxt": 1000
            },
            "t6": {
                "color": [
                    0.21,
                    0.29,
                    0.27,
                    0
                ],
                "maxt": 1500
            },
            "t7": {
                "color": [
                    0.19,
                    0.23,
                    0.21,
                    0
                ],
                "maxt": 2000
            },
            "t8": {
                "color": [
                    0.22,
                    0.19,
                    0.1,
                    0
                ],
                "maxt": 2300
            },
            "t9": {
                "color": [
                    0.35,
                    0.2,
                    0.02,
                    0
                ],
                "maxt": 2500
            }
        },
        "timetolive": 0
    },
    "gunnercansee": "4+8+16",
    "gunnerhasflares": 1,
    "hasdriver": 0,
    "headaimdown": 0,
    "headgforceleaningfactor": [
        0.01,
        0.002,
        0.01
    ],
    "headlimits": {
        "initanglex": 5,
        "initangley": 0,
        "maxanglex": 40,
        "maxangley": 90,
        "maxanglez": 45,
        "minanglex": -30,
        "minangley": -90,
        "minanglez": -45,
        "rotzradius": 0.2
    },
    "hiddenselections": [],
    "hiddenselectionsmaterials": [],
    "hiddenselectionstextures": [],
    "hiddenunderwaterselections": [],
    "hiddenunderwaterselectionstextures": [],
    "hideproxyincombat": 0,
    "hideunitinfo": 0,
    "hideweaponscargo": 0,
    "hideweaponsdriver": 1,
    "hitpoints": {
        "glass_10_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_10_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_10_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_1250_2200"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_10_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_1250_2200"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_10_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_1250_2200"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_10_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_1250_2200"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_10_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_1250_2200"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_10_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_1250_2200"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_10_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_1250_2200"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_10_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_1250_2200"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_10_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_1250_2200"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_10_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_1250_2200"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_10_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_1250_2200"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_10_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_1250_2200"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_10_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_1250_2200"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_10_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_1250_2200"
                }
            },
            "material": -1,
            "name": "Glass_10",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_10_hide"
        },
        "glass_11_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_11_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_11_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_1250_2200"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_11_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_1250_2200"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_11_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_1250_2200"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_11_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_1250_2200"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_11_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_1250_2200"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_11_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_1250_2200"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_11_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_1250_2200"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_11_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_1250_2200"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_11_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_1250_2200"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_11_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_1250_2200"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_11_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_1250_2200"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_11_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_1250_2200"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_11_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_1250_2200"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_11_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_1250_2200"
                }
            },
            "material": -1,
            "name": "Glass_11",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_11_hide"
        },
        "glass_12_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_12_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_12_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_1250_2200"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_12_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_1250_2200"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_12_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_1250_2200"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_12_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_1250_2200"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_12_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_1250_2200"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_12_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_1250_2200"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_12_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_1250_2200"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_12_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_1250_2200"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_12_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_1250_2200"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_12_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_1250_2200"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_12_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_1250_2200"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_12_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_1250_2200"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_12_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_1250_2200"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_12_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_1250_2200"
                }
            },
            "material": -1,
            "name": "Glass_12",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_12_hide"
        },
        "glass_13_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_13_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_13_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_1150_0600"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_13_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_1150_0600"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_13_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_1150_0600"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_13_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_1150_0600"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_13_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_1150_0600"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_13_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_1150_0600"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_13_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_1150_0600"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_13_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_1150_0600"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_13_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_1150_0600"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_13_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_1150_0600"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_13_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_1150_0600"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_13_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_1150_0600"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_13_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_1150_0600"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_13_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_1150_0600"
                }
            },
            "material": -1,
            "name": "Glass_13",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_13_hide"
        },
        "glass_14_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_14_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_14_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_1150_0600"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_14_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_1150_0600"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_14_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_1150_0600"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_14_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_1150_0600"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_14_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_1150_0600"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_14_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_1150_0600"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_14_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_1150_0600"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_14_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_1150_0600"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_14_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_1150_0600"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_14_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_1150_0600"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_14_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_1150_0600"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_14_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_1150_0600"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_14_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_1150_0600"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_14_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_1150_0600"
                }
            },
            "material": -1,
            "name": "Glass_14",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_14_hide"
        },
        "glass_15_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_15_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_15_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_1150_0600"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_15_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_1150_0600"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_15_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_1150_0600"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_15_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_1150_0600"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_15_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_1150_0600"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_15_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_1150_0600"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_15_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_1150_0600"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_15_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_1150_0600"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_15_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_1150_0600"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_15_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_1150_0600"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_15_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_1150_0600"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_15_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_1150_0600"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_15_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_1150_0600"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_15_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_1150_0600"
                }
            },
            "material": -1,
            "name": "Glass_15",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_15_hide"
        },
        "glass_16_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_16_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_16_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_1150_0600"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_16_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_1150_0600"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_16_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_1150_0600"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_16_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_1150_0600"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_16_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_1150_0600"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_16_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_1150_0600"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_16_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_1150_0600"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_16_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_1150_0600"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_16_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_1150_0600"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_16_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_1150_0600"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_16_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_1150_0600"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_16_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_1150_0600"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_16_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_1150_0600"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_16_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_1150_0600"
                }
            },
            "material": -1,
            "name": "Glass_16",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_16_hide"
        },
        "glass_17_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_17_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_17_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_1150_0600"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_17_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_1150_0600"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_17_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_1150_0600"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_17_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_1150_0600"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_17_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_1150_0600"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_17_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_1150_0600"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_17_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_1150_0600"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_17_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_1150_0600"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_17_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_1150_0600"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_17_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_1150_0600"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_17_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_1150_0600"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_17_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_1150_0600"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_17_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_1150_0600"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_17_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_1150_0600"
                }
            },
            "material": -1,
            "name": "Glass_17",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_17_hide"
        },
        "glass_18_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_18_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_18_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_1150_0600"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_18_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_1150_0600"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_18_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_1150_0600"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_18_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_1150_0600"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_18_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_1150_0600"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_18_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_1150_0600"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_18_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_1150_0600"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_18_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_1150_0600"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_18_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_1150_0600"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_18_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_1150_0600"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_18_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_1150_0600"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_18_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_1150_0600"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_18_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_1150_0600"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_18_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_1150_0600"
                }
            },
            "material": -1,
            "name": "Glass_18",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_18_hide"
        },
        "glass_19_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_19_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_19_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_0900_1300"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_19_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_0900_1300"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_19_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_0900_1300"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_19_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_0900_1300"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_19_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_0900_1300"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_19_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_0900_1300"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_19_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_0900_1300"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_19_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_0900_1300"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_19_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_0900_1300"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_19_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_0900_1300"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_19_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_0900_1300"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_19_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_0900_1300"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_19_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_0900_1300"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_19_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_0900_1300"
                }
            },
            "material": -1,
            "name": "Glass_19",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_19_hide"
        },
        "glass_1_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_1_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_1_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_1150_0600"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_1_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_1150_0600"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_1_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_1150_0600"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_1_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_1150_0600"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_1_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_1150_0600"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_1_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_1150_0600"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_1_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_1150_0600"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_1_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_1150_0600"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_1_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_1150_0600"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_1_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_1150_0600"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_1_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_1150_0600"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_1_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_1150_0600"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_1_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_1150_0600"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_1_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_1150_0600"
                }
            },
            "material": -1,
            "name": "Glass_1",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_1_hide"
        },
        "glass_20_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_20_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_20_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_0900_1300"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_20_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_0900_1300"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_20_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_0900_1300"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_20_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_0900_1300"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_20_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_0900_1300"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_20_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_0900_1300"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_20_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_0900_1300"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_20_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_0900_1300"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_20_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_0900_1300"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_20_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_0900_1300"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_20_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_0900_1300"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_20_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_0900_1300"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_20_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_0900_1300"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_20_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_0900_1300"
                }
            },
            "material": -1,
            "name": "Glass_20",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_20_hide"
        },
        "glass_21_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_21_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_21_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_0900_1300"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_21_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_0900_1300"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_21_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_0900_1300"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_21_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_0900_1300"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_21_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_0900_1300"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_21_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_0900_1300"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_21_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_0900_1300"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_21_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_0900_1300"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_21_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_0900_1300"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_21_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_0900_1300"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_21_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_0900_1300"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_21_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_0900_1300"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_21_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_0900_1300"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_21_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_0900_1300"
                }
            },
            "material": -1,
            "name": "Glass_21",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_21_hide"
        },
        "glass_2_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_2_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_2_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_0900_1300"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_2_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_0900_1300"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_2_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_0900_1300"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_2_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_0900_1300"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_2_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_0900_1300"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_2_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_0900_1300"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_2_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_0900_1300"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_2_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_0900_1300"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_2_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_0900_1300"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_2_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_0900_1300"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_2_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_0900_1300"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_2_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_0900_1300"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_2_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_0900_1300"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_2_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_0900_1300"
                }
            },
            "material": -1,
            "name": "Glass_2",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_2_hide"
        },
        "glass_3_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_3_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_3_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_0900_1300"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_3_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_0900_1300"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_3_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_0900_1300"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_3_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_0900_1300"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_3_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_0900_1300"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_3_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_0900_1300"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_3_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_0900_1300"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_3_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_0900_1300"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_3_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_0900_1300"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_3_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_0900_1300"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_3_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_0900_1300"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_3_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_0900_1300"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_3_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_0900_1300"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_3_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_0900_1300"
                }
            },
            "material": -1,
            "name": "Glass_3",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_3_hide"
        },
        "glass_4_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_4_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_4_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_0900_1300"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_4_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_0900_1300"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_4_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_0900_1300"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_4_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_0900_1300"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_4_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_0900_1300"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_4_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_0900_1300"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_4_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_0900_1300"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_4_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_0900_1300"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_4_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_0900_1300"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_4_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_0900_1300"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_4_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_0900_1300"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_4_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_0900_1300"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_4_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_0900_1300"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_4_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_0900_1300"
                }
            },
            "material": -1,
            "name": "Glass_4",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_4_hide"
        },
        "glass_5_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_5_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_5_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_0900_1300"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_5_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_0900_1300"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_5_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_0900_1300"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_5_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_0900_1300"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_5_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_0900_1300"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_5_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_0900_1300"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_5_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_0900_1300"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_5_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_0900_1300"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_5_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_0900_1300"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_5_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_0900_1300"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_5_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_0900_1300"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_5_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_0900_1300"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_5_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_0900_1300"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_5_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_0900_1300"
                }
            },
            "material": -1,
            "name": "Glass_5",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_5_hide"
        },
        "glass_6_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_6_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_6_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_0900_1300"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_6_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_0900_1300"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_6_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_0900_1300"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_6_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_0900_1300"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_6_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_0900_1300"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_6_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_0900_1300"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_6_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_0900_1300"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_6_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_0900_1300"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_6_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_0900_1300"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_6_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_0900_1300"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_6_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_0900_1300"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_6_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_0900_1300"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_6_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_0900_1300"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_6_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_0900_1300"
                }
            },
            "material": -1,
            "name": "Glass_6",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_6_hide"
        },
        "glass_7_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_7_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_7_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_1250_2200"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_7_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_1250_2200"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_7_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_1250_2200"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_7_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_1250_2200"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_7_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_1250_2200"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_7_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_1250_2200"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_7_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_1250_2200"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_7_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_1250_2200"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_7_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_1250_2200"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_7_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_1250_2200"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_7_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_1250_2200"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_7_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_1250_2200"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_7_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_1250_2200"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_7_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_1250_2200"
                }
            },
            "material": -1,
            "name": "Glass_7",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_7_hide"
        },
        "glass_8_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_8_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_8_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_1250_2200"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_8_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_1250_2200"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_8_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_1250_2200"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_8_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_1250_2200"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_8_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_1250_2200"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_8_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_1250_2200"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_8_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_1250_2200"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_8_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_1250_2200"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_8_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_1250_2200"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_8_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_1250_2200"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_8_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_1250_2200"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_8_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_1250_2200"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_8_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_1250_2200"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_8_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_1250_2200"
                }
            },
            "material": -1,
            "name": "Glass_8",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_8_hide"
        },
        "glass_9_hitpoint": {
            "armor": 0.001,
            "convexcomponent": "Glass_9_hide",
            "destructioneffects": {
                "brokenglass1": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_9_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1N_1250_2200"
                },
                "brokenglass1s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_9_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass1S_1250_2200"
                },
                "brokenglass2": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_9_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2N_1250_2200"
                },
                "brokenglass2s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_9_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass2S_1250_2200"
                },
                "brokenglass3": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_9_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3N_1250_2200"
                },
                "brokenglass3s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_9_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass3S_1250_2200"
                },
                "brokenglass4": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_9_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4N_1250_2200"
                },
                "brokenglass4s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_9_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass4S_1250_2200"
                },
                "brokenglass5": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_9_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5N_1250_2200"
                },
                "brokenglass5s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_9_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass5S_1250_2200"
                },
                "brokenglass6": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_9_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6N_1250_2200"
                },
                "brokenglass6s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_9_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass6S_1250_2200"
                },
                "brokenglass7": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_9_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7N_1250_2200"
                },
                "brokenglass7s": {
                    "intensity": 0.15,
                    "interval": 1,
                    "lifetime": 0.05,
                    "position": "Glass_9_effects",
                    "simulation": "particles",
                    "type": "BrokenGlass7S_1250_2200"
                }
            },
            "material": -1,
            "name": "Glass_9",
            "passthrough": 0,
            "radius": 0.4,
            "visual": "Glass_9_hide"
        }
    },
    "hulldamagecauseexplosion": 0,
    "icon": "iconObject_1x1",
    "impacteffectssea": "ImpactEffectsSea",
    "incomingmissiledetectionsystem": 0,
    "indirecthitciviliancoefai": -20,
    "indirecthitenemycoefai": 10,
    "indirecthitfriendlycoefai": -20,
    "indirecthitunknowncoefai": -0.5,
    "insidedetectcoef": 0.05,
    "insidesoundcoef": 0.5,
    "irscanground": 1,
    "irscanrangemax": 0,
    "irscanrangemin": 0,
    "irscantoeyefactor": 1,
    "irtarget": 0,
    "isbackpack": 0,
    "islockingdisabled": 1,
    "keephorizontalplacement": 1,
    "keepinepesceneafterdeath": 0,
    "killfriendlyexpcoef": 1,
    "ladders": [],
    "laserscanner": 0,
    "lasertarget": 0,
    "limitedspeedcoef": 0.22,
    "lockdetectionsystem": 0,
    "magazines": [],
    "mapsize": 17.16,
    "markerlights": {},
    "maxdetectrange": 20,
    "maxfordingdepth": 1,
    "maxgforce": 2,
    "maxspeed": 0,
    "memorypointsupply": "doplnovani",
    "mgunclouds": {
        "access": 0,
        "cloudletaccy": 0,
        "cloudletalpha": 0.3,
        "cloudletanimperiod": 1,
        "cloudletcolor": [
            1,
            1,
            1,
            0
        ],
        "cloudletduration": 0.05,
        "cloudletfadein": 0,
        "cloudletfadeout": 0.1,
        "cloudletgrowup": 0.05,
        "cloudletmaxyspeed": 100,
        "cloudletminyspeed": -100,
        "cloudletshape": "cloudletClouds",
        "cloudletsize": 1,
        "deltat": 0,
        "initt": 0,
        "interval": 0.02,
        "size": 0.3,
        "sourcesize": 0.02,
        "table": {
            "t0": {
                "color": [
                    1,
                    1,
                    1,
                    0
                ],
                "maxt": 0
            }
        },
        "timetolive": 0
    },
    "minealerticonrange": 200,
    "minfiretime": 20,
    "mingforce": 0.2,
    "mintotaldamagethreshold": 0.01,
    "model": "A3/Structures_F_Argo/Commercial/Shop_02/i_Shop_02_b_yellow_F.p3d",
    "namesound": "obj_building",
    "nightvision": 0,
    "normalspeedforwardcoef": 0.85,
    "numberofdoors": 4,
    "numberofwindows": 18,
    "numberphysicalwheels": 0,
    "nvgmarker": {
        "ambient": [
            1,
            1,
            1,
            1
        ],
        "blinking": 0,
        "brightness": 1,
        "diffuse": [
            1,
            1,
            1,
            1
        ],
        "onlyinnvg": 0
    },
    "nvgmarkers": {},
    "nvscanner": 0,
    "nvtarget": 0,
    "obstructsoundlfratio": 0,
    "obstructsoundswhenin": 0.562341,
    "occludesoundlfratio": 0.25,
    "occludesoundswhenin": 0.316228,
    "outsidesoundfilter": 0,
    "picture": "pictureStaticObject",
    "pilotspec": {
        "showheadphones": 0
    },
    "portrait": "",
    "precision": 5,
    "predictturnplan": 1,
    "predictturnsimul": 1.2,
    "preferroads": 0,
    "pulsationsound": {},
    "radartype": 0,
    "reflectors": {},
    "replacedamaged": "",
    "replacedamagedhitpoints": [],
    "replacedamagedlimit": 0.99999,
    "reversed": 0,
    "scope": 2,
    "scopecurator": 2,
    "secondaryexplosion": -1,
    "selectionbacklights": "",
    "selectionclan": "",
    "selectiondamage": "DamT_0",
    "selectiondashboard": "",
    "selectionfireanim": "",
    "selectionshowdamage": "",
    "sensitivity": 2.5,
    "sensitivityear": 0.0075,
    "shadow": 1,
    "showalltargets": 0,
    "showcrewaim": 7,
    "shownunderwaterselections": [],
    "shownvgcargo": [
        0
    ],
    "shownvgcommander": 0,
    "shownvgdriver": 0,
    "shownvggunner": 0,
    "side": 3,
    "simpleobject": {
        "animate": [
            [
                "door_1_rot",
                0
            ],
            [
                "door_1_handle_rot_1",
                0
            ],
            [
                "door_1_handle_rot_2",
                0
            ],
            [
                "door_1_locked_rot",
                0
            ],
            [
                "door_1_handle_locked_rot",
                0
            ],
            [
                "door_2_rot",
                0
            ],
            [
                "door_2_handle_rot_1",
                0
            ],
            [
                "door_2_handle_rot_2",
                0
            ],
            [
                "door_2_locked_rot",
                0
            ],
            [
                "door_2_handle_locked_rot",
                0
            ],
            [
                "door_3_rot",
                0
            ],
            [
                "door_3_handle_rot_1",
                0
            ],
            [
                "door_3_handle_rot_2",
                0
            ],
            [
                "door_3_locked_rot",
                0
            ],
            [
                "door_3_handle_locked_rot",
                0
            ],
            [
                "door_4_rot",
                0
            ],
            [
                "door_4_handle_rot_1",
                0
            ],
            [
                "door_4_handle_rot_2",
                0
            ],
            [
                "door_4_locked_rot",
                0
            ],
            [
                "door_4_handle_locked_rot",
                0
            ],
            [
                "glass_1_hide",
                0
            ],
            [
                "glass_1_unhide",
                0
            ],
            [
                "glass_2_hide",
                0
            ],
            [
                "glass_2_unhide",
                0
            ],
            [
                "glass_3_hide",
                0
            ],
            [
                "glass_3_unhide",
                0
            ],
            [
                "glass_4_hide",
                0
            ],
            [
                "glass_4_unhide",
                0
            ],
            [
                "glass_5_hide",
                0
            ],
            [
                "glass_5_unhide",
                0
            ],
            [
                "glass_6_hide",
                0
            ],
            [
                "glass_6_unhide",
                0
            ],
            [
                "glass_7_hide",
                0
            ],
            [
                "glass_7_unhide",
                0
            ],
            [
                "glass_8_hide",
                0
            ],
            [
                "glass_8_unhide",
                0
            ],
            [
                "glass_9_hide",
                0
            ],
            [
                "glass_9_unhide",
                0
            ],
            [
                "glass_10_hide",
                0
            ],
            [
                "glass_10_unhide",
                0
            ],
            [
                "glass_11_hide",
                0
            ],
            [
                "glass_11_unhide",
                0
            ],
            [
                "glass_12_hide",
                0
            ],
            [
                "glass_12_unhide",
                0
            ],
            [
                "glass_13_hide",
                0
            ],
            [
                "glass_13_unhide",
                0
            ],
            [
                "glass_14_hide",
                0
            ],
            [
                "glass_14_unhide",
                0
            ],
            [
                "glass_15_hide",
                0
            ],
            [
                "glass_15_unhide",
                0
            ],
            [
                "glass_16_hide",
                0
            ],
            [
                "glass_16_unhide",
                0
            ],
            [
                "glass_17_hide",
                0
            ],
            [
                "glass_17_unhide",
                0
            ],
            [
                "glass_18_hide",
                0
            ],
            [
                "glass_18_unhide",
                0
            ],
            [
                "glass_19_hide",
                0
            ],
            [
                "glass_19_unhide",
                0
            ],
            [
                "glass_20_hide",
                0
            ],
            [
                "glass_20_unhide",
                0
            ],
            [
                "glass_21_hide",
                0
            ],
            [
                "glass_21_unhide",
                0
            ]
        ],
        "eden": 0,
        "hide": [],
        "init": "''",
        "verticaloffset": 3.18,
        "verticaloffsetworld": 0
    },
    "simulation": "house",
    "slingloadcargomemorypoints": [],
    "slingloadcargomemorypointsdir": [],
    "slowspeedforwardcoef": 0.3,
    "soundarmorcrash": [
        "emptySound",
        0
    ],
    "soundattenuationcargo": [
        1
    ],
    "soundbreath": {},
    "soundbreathautomatic": {},
    "soundbreathinjured": {},
    "soundbreathswimming": {},
    "soundbuildingcrash": [
        "emptySound",
        0
    ],
    "soundburning": {},
    "soundbushcrash": [
        "emptySound",
        0
    ],
    "soundchoke": {},
    "soundcrash": [
        "",
        0.316228,
        1
    ],
    "soundcrashes": [
        "soundCrash",
        1
    ],
    "sounddamage": [
        "",
        1,
        1
    ],
    "sounddammage": [
        "",
        1,
        1
    ],
    "sounddrown": {},
    "sounddrowning": {},
    "soundelevation": [
        "",
        0.00316228,
        0.5
    ],
    "soundengine": [
        "",
        1,
        1
    ],
    "soundengineoffext": [
        "",
        1,
        1
    ],
    "soundengineoffint": [
        "",
        1,
        1
    ],
    "soundengineonext": [
        "",
        1,
        1
    ],
    "soundengineonint": [
        "",
        1,
        1
    ],
    "soundenviron": [
        "",
        1,
        1
    ],
    "soundenvironext": {},
    "soundequipment": {},
    "soundflapsdown": [
        "",
        1,
        1
    ],
    "soundflapsup": [
        "",
        1,
        1
    ],
    "soundgear": {},
    "soundgeardown": [
        "",
        1,
        1
    ],
    "soundgearup": [
        "",
        1,
        1
    ],
    "soundgetin": [
        "",
        0.000316228,
        1
    ],
    "soundgetout": [
        "",
        0.000316228,
        1
    ],
    "soundhitscream": {},
    "soundincommingmissile": [
        "",
        1,
        1
    ],
    "soundinjured": {},
    "soundlandcrash": [
        "",
        1,
        1
    ],
    "soundlandcrashes": [
        "soundLandCrash",
        1
    ],
    "soundlocked": [
        "",
        1,
        1
    ],
    "soundrecovered": {},
    "soundservo": [
        "",
        0.00316228,
        0.5
    ],
    "soundturnin": [
        "",
        0.000316228,
        1
    ],
    "soundturnininternal": [
        "",
        0.000316228,
        1
    ],
    "soundturnout": [
        "",
        0.000316228,
        1
    ],
    "soundturnoutinternal": [
        "",
        0.000316228,
        1
    ],
    "soundwatercrash": [
        "",
        0.177828,
        1
    ],
    "soundwatercrashes": [
        "soundWaterCrash",
        1
    ],
    "soundwoodcrash": [
        "emptySound",
        0
    ],
    "speechplural": [],
    "speechsingular": [],
    "speechvariants": {
        "default": {
            "speechplural": [
                "veh_unknown_p"
            ],
            "speechsingular": [
                "veh_unknown_s"
            ]
        }
    },
    "spotabledarknightlightsoff": 0.001,
    "spotablenightlightsoff": 0.02,
    "spotablenightlightson": 4,
    "steeraheadplan": 0.4,
    "steeraheadsimul": 0.3,
    "supplyradius": -1,
    "textplural": "unknown",
    "textsingular": "unknown",
    "threat": [
        0,
        0,
        0
    ],
    "transportammo": 0,
    "transportfuel": 0,
    "transportmagazines": {},
    "transportmaxbackpacks": 0,
    "transportmaxmagazines": 0,
    "transportmaxweapons": 0,
    "transportrepair": 0,
    "transportsoldier": 0,
    "transportvehiclescount": 0,
    "transportvehiclesmass": 0,
    "transportweapons": {},
    "turrets": {},
    "type": 1,
    "typicalcargo": [],
    "uavhacker": 0,
    "unitinfotype": "RscUnitInfoTank",
    "unitinfotypelite": 0,
    "unloadincombat": 0,
    "useprecisegetinaction": 0,
    "useractions": {
        "closedoor_1": {
            "actionnamedsel": "Door_1_action",
            "aimaxrange": 5.25,
            "condition": "((this animationSourcePhase 'Door_1_sound_source') >= 0.5) && ((this getVariable ['bis_disabled_Door_1', 0]) != 1) && (cameraOn isKindOf 'CAManBase') && ((this animationSourcePhase 'Hitzone_2_source') < 0.99999\t\t\t\t)",
            "displayname": "Close door",
            "displaynamedefault": "<img image='/A3/Ui_f/data/IGUI/Cfg/Actions/open_door_ca.paa' size='2.5' />",
            "onlyforplayer": 0,
            "position": "Door_1_trigger",
            "priority": 11,
            "radius": 1.75,
            "statement": "([this, 1, 0] call BIS_fnc_Door)"
        },
        "closedoor_2": {
            "actionnamedsel": "Door_2_action",
            "aimaxrange": 5.25,
            "condition": "((this animationSourcePhase 'Door_2_sound_source') >= 0.5) && ((this getVariable ['bis_disabled_Door_2', 0]) != 1) && (cameraOn isKindOf 'CAManBase')",
            "displayname": "Close door",
            "displaynamedefault": "<img image='/A3/Ui_f/data/IGUI/Cfg/Actions/open_door_ca.paa' size='2.5' />",
            "onlyforplayer": 0,
            "position": "Door_2_trigger",
            "priority": 11,
            "radius": 1.75,
            "statement": "([this, 2, 0] call BIS_fnc_Door)"
        },
        "closedoor_3": {
            "actionnamedsel": "Door_3_action",
            "aimaxrange": 5.25,
            "condition": "((this animationSourcePhase 'Door_3_sound_source') >= 0.5) && ((this getVariable ['bis_disabled_Door_3', 0]) != 1) && (cameraOn isKindOf 'CAManBase') && ((this animationSourcePhase 'Hitzone_1_source') < 0.99999\t\t\t\t)",
            "displayname": "Close door",
            "displaynamedefault": "<img image='/A3/Ui_f/data/IGUI/Cfg/Actions/open_door_ca.paa' size='2.5' />",
            "onlyforplayer": 0,
            "position": "Door_3_trigger",
            "priority": 11,
            "radius": 1.75,
            "statement": "([this, 3, 0] call BIS_fnc_Door)"
        },
        "closedoor_4": {
            "actionnamedsel": "Door_4_action",
            "aimaxrange": 5.25,
            "condition": "((this animationSourcePhase 'Door_4_sound_source') >= 0.5) && ((this getVariable ['bis_disabled_Door_4', 0]) != 1) && (cameraOn isKindOf 'CAManBase') && ((this animationSourcePhase 'Hitzone_2_source') < 0.99999\t\t\t\t)",
            "displayname": "Close door",
            "displaynamedefault": "<img image='/A3/Ui_f/data/IGUI/Cfg/Actions/open_door_ca.paa' size='2.5' />",
            "onlyforplayer": 0,
            "position": "Door_4_trigger",
            "priority": 11,
            "radius": 1.75,
            "statement": "([this, 4, 0] call BIS_fnc_Door)"
        },
        "opendoor_1": {
            "actionnamedsel": "Door_1_action",
            "aimaxrange": 5.25,
            "condition": "((this animationSourcePhase 'Door_1_sound_source') < 0.5) && (cameraOn isKindOf 'CAManBase') && ((this animationSourcePhase 'Hitzone_2_source') < 0.99999\t\t\t\t)",
            "displayname": "Open door",
            "displaynamedefault": "<img image='/A3/Ui_f/data/IGUI/Cfg/Actions/open_door_ca.paa' size='2.5' />",
            "onlyforplayer": 0,
            "position": "Door_1_trigger",
            "priority": 11,
            "radius": 1.75,
            "statement": "([this, 1, 1] call BIS_fnc_Door)"
        },
        "opendoor_2": {
            "actionnamedsel": "Door_2_action",
            "aimaxrange": 5.25,
            "condition": "((this animationSourcePhase 'Door_2_sound_source') < 0.5) && (cameraOn isKindOf 'CAManBase')",
            "displayname": "Open door",
            "displaynamedefault": "<img image='/A3/Ui_f/data/IGUI/Cfg/Actions/open_door_ca.paa' size='2.5' />",
            "onlyforplayer": 0,
            "position": "Door_2_trigger",
            "priority": 11,
            "radius": 1.75,
            "statement": "([this, 2, 1] call BIS_fnc_Door)"
        },
        "opendoor_3": {
            "actionnamedsel": "Door_3_action",
            "aimaxrange": 5.25,
            "condition": "((this animationSourcePhase 'Door_3_sound_source') < 0.5) && (cameraOn isKindOf 'CAManBase') && ((this animationSourcePhase 'Hitzone_1_source') < 0.99999\t\t\t\t)",
            "displayname": "Open door",
            "displaynamedefault": "<img image='/A3/Ui_f/data/IGUI/Cfg/Actions/open_door_ca.paa' size='2.5' />",
            "onlyforplayer": 0,
            "position": "Door_3_trigger",
            "priority": 11,
            "radius": 1.75,
            "statement": "([this, 3, 1] call BIS_fnc_Door)"
        },
        "opendoor_4": {
            "actionnamedsel": "Door_4_action",
            "aimaxrange": 5.25,
            "condition": "((this animationSourcePhase 'Door_4_sound_source') < 0.5) && (cameraOn isKindOf 'CAManBase') && ((this animationSourcePhase 'Hitzone_2_source') < 0.99999\t\t\t\t)",
            "displayname": "Open door",
            "displaynamedefault": "<img image='/A3/Ui_f/data/IGUI/Cfg/Actions/open_door_ca.paa' size='2.5' />",
            "onlyforplayer": 0,
            "position": "Door_4_trigger",
            "priority": 11,
            "radius": 1.75,
            "statement": "([this, 4, 1] call BIS_fnc_Door)"
        }
    },
    "vehicleclass": "Structures_Commercial",
    "viewcargo": {
        "initanglex": 5,
        "initangley": 0,
        "initfov": 0.75,
        "maxanglex": 85,
        "maxangley": 150,
        "maxfov": 1.25,
        "maxmovex": 0,
        "maxmovey": 0,
        "maxmovez": 0,
        "minanglex": -85,
        "minangley": -150,
        "minfov": 0.25,
        "minmovex": 0,
        "minmovey": 0,
        "minmovez": 0,
        "speedzoommaxfov": 0,
        "speedzoommaxspeed": 10000000000.0
    },
    "viewcargoshadow": 1,
    "viewcargoshadowamb": 1,
    "viewcargoshadowdiff": 1,
    "viewdrivershadow": 1,
    "viewdrivershadowamb": 1,
    "viewdrivershadowdiff": 1,
    "viewoptics": {
        "initanglex": 0,
        "initangley": 0,
        "initfov": 0.7,
        "maxanglex": 30,
        "maxangley": 100,
        "maxfov": 0.85,
        "maxmovex": 0,
        "maxmovey": 0,
        "maxmovez": 0,
        "minanglex": -30,
        "minangley": -100,
        "minfov": 0.42,
        "minmovex": 0,
        "minmovey": 0,
        "minmovez": 0,
        "speedzoommaxfov": 0,
        "speedzoommaxspeed": 10000000000.0
    },
    "viewpilot": {
        "initanglex": 5,
        "initangley": 0,
        "initfov": 0.75,
        "maxanglex": 85,
        "maxangley": 150,
        "maxfov": 1.25,
        "maxmovex": 0,
        "maxmovey": 0,
        "maxmovez": 0,
        "minanglex": -55,
        "minangley": -150,
        "minfov": 0.25,
        "minmovex": 0,
        "minmovey": 0,
        "minmovez": 0,
        "speedzoommaxfov": 0,
        "speedzoommaxspeed": 10000000000.0
    },
    "waterangulardampingcoef": 0,
    "waterdamageengine": 0.2,
    "waterleakiness": 0.5,
    "waterlineardampingcoefx": 0,
    "waterlineardampingcoefy": 0,
    "waterresistance": 10,
    "waterresistancecoef": 0.5,
    "weapons": [],
    "weaponsgroup1": "1 + 2",
    "weaponsgroup2": 4,
    "weaponsgroup3": "8 + 16 + 32",
    "weaponsgroup4": "64 + 128",
    "weaponslots": 0,
    "wheelcircumference": 1,
    "windsockexist": 0
}