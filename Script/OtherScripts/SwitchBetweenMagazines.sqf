...
upTenMag = (findDisplay 46) displayAddEventHandler ["KeyDown", "if ((_this select 1) == 6) then {
		if (x <= 1490) then {
			x = x + 10; [x] call changeMag;
		};
	};"];
downTenMag = (findDisplay 46) displayAddEventHandler ["KeyDown", "if ((_this select 1) == 7) then {
		if (x > 10) then {
			x = x - 10; [x] call changeMag;
		};
	};"];
