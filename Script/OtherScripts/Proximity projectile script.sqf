...
bulletA = player addAction ["Enable Prox", {firedEH = vehicle player addEventHandler ["Fired", {
	...
	*/
	...
	if(_proximityMode isEqualTo 1) then {
		...
		[_pfhTop, "onEachFrame", {
			params["_p","_vehList","_pfhTop","_proximityRange"];
			private _tgtList = _vehList select {_p distance _x < (_proximityRange * 10)};
			{
				diag_log(str(_p distance _x) + str(_x));
				//If projectile is in prox range
				if (_p distance _x <= _proximityRange) exitWith {
					hint str (_p distance _x);
		...
		}, [_p,_vehList,_pfhTop,_proximityRange]] call BIS_fnc_addStackedEventHandler;
	};
}]}];
