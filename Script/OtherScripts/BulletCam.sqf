		...
		while {true} do {
			if ((velocity _missile) isNotEqualTo velCache) then {
				if (!(isNull _missile)) then {
					...
					systemChat format["Distance: %1 | Velocity: %2 | Pos: %3", player distance2D _missile, velocity _missile, getPos _missile, isNull _missile];
				} else {
					break;
				}
			}
		...
		while {true} do {
			if ((velocity _missile) isNotEqualTo velCache) then {
				if (!(isNull _missile)) then {
					...
					systemChat format["x: %1| y: %2|| z: %3", xsqrd, ysqrd, zsqrd];
					//diag_log format["Distance: %1 | Velocity: %2 | Pos: %3", player distance2D _missile, velocity _missile, getPos _missile];
					systemChat format["Distance: %1 | Velocity: %2 | Pos: %3", player distance2D _missile, velocity _missile select 1, getPos _missile, isNull _missile];
				} else {
					break;
				}
			}
