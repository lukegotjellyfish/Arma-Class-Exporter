...
_basePath = "C:\Users\Lukeg\Desktop\GitHub\MyRepos\Arma-Class-Exporter\Exports\SQF-Class-Exports\RHS\";
...
_mods = [["CfgWeapons","Weapons"],["CfgMagazines","Magazines"],["CfgVehicles","Vehicles"],["CfgGlasses","Glasses"],["CfgAmmo","Ammo"]]; //["CfgWeapons","Weapons"],["CfgMagazines","Magazines"],["CfgVehicles","Vehicles"],["CfgGlasses","Glasses"]
{
    ...
    _rhsusaf = "'@RHSUSAF' in (configSourceModList _x)" configClasses (configFile >> _configClass) apply {configName _x};
    _rhsgref = "'@RHSGREF' in (configSourceModList _x)" configClasses (configFile >> _configClass) apply {configName _x};
    _rhssaf  = "'@RHSSAF' in (configSourceModList _x)"  configClasses (configFile >> _configClass) apply {configName _x};
    
    //Count number of classes in each mod for given CfgClass
    _rhsafrfClasses = count(_rhsafrf);
    _rhsusafClasses = count(_rhsusaf);
    _rhsgrefClasses = count(_rhsgref);
    _rhssafClasses = count(_rhssaf);

    //Calculate total number of classes to export
    _totalClasses = str (_rhsafrfClasses + _rhsusafClasses + _rhsgrefClasses + _rhssafClasses);

    //Convert number of classes to string for GUIMessage
    _rhsafrfClasses = str _rhsafrfClasses;
    _rhsusafClasses = str _rhsusafClasses;
    _rhsgrefClasses = str _rhsgrefClasses;
    _rhssafClasses = str (_rhssafClasses);

    //Compose message to allow for line breaks
    //_message = composeText ["Do you want to export ",_totalClasses," Classes from ",_configClass,"?",lineBreak,"RHSAFRF: ", _rhsafrfClasses, lineBreak,"RHSUSAF: ", _rhsusafClasses, lineBreak, "RHSGREF: ", _rhsgrefClasses, lineBreak, "RHSSAF: ", _rhssafClasses];
    //Display GUIMessage and save the result [true/false] for OK or NO respectively
    //_result = [_message,"Export Confirmation", "        =Yes=", "        =No="] call BIS_fnc_GUIMessage;
    _result = True;

    //If OK is pressed, then continue to export all CfgClasses in given mods as _result is TRUE
    if (_result) then {
        _config = [];
